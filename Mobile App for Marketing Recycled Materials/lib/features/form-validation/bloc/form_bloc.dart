...
import 'package:trial/features/database/database_repository_impl.dart';
...
class FormBloc extends Bloc<FormEvent, FormsValidate> {
  ...
  FormBloc(this._authenticationRepository, this._databaseRepository)
    ...
    on<UserCatChanged>(_onUserCatChanged);
  ...
  bool _isUserCatValid(String? userCategory) {
    return userCategory!.isNotEmpty;
  }
  ...
  _onUserCatChanged(UserCatChanged event, Emitter<FormsValidate> emit) {
    emit(state.copyWith(
      isFormSuccessful: false,
      isFormValidateFailed: false,
      errorMessage: "",
      userCategory: event.userCategory,
      isUserCatValid: _isUserCatValid(event.userCategory),
    ));
  }
  ...
  _updateUIAndSignUp(
    ...
    emit(state.copyWith(
        ...
        isFormValid: _isPasswordValid(state.password) &&
            ...
            _isUserCatValid(state.userCategory),
        isLoading: true));
  ...
  _authenticateUser(
    ...
    if (state.isFormValid) {
      ...
      emit(state.copyWith(
          isLoading: false, isFormValid: false, isFormValidateFailed: false));
    }
  }
